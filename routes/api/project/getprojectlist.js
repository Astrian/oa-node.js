...
module.exports = function (req, res, api, 请求体) {
  回调函数是一个反人类的东西(function* (回调) {
    ...
    if (请求体.type == 1) { // 获取需要我处理的专案
      ...
      SQL语句 = 'SELECT 申请人, 申请时间, 模板, 状态 FROM project WHERE 目前处理的部门 = ' + 数据库结果[0].所属部门
      请求结果 = yield 调用数据库(SQL语句, 回调.next)
    }
    else if (请求体.type == 2) {
      SQL语句 = 'SELECT 申请人, 申请时间, 模板, 状态 FROM project WHERE 申请人 = ' + 登录用户
      请求结果 = yield 调用数据库(SQL语句, 回调.next)
    }
    for(var i in 请求结果){
      ...
      数据库结果 = yield 调用数据库(SQL语句, 回调.next)
      请求结果[i].模板 = 数据库结果[i].标题
      SQL语句 = 'SELECT 姓, 名 FROM user WHERE id = '+请求结果[i].申请人
      数据库结果 = yield 调用数据库(SQL语句, 回调.next)
      请求结果[i].申请人 = (数据库结果[i].姓).concat(数据库结果[i].名)
    }
    成功返回(请求结果)
  })
}